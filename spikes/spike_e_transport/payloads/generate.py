...
REAL PAYLOAD PROFILES FROM SPIKE D
----------------------------------
Spike D A6 reported uint8 volumes in two observed shapes: 69 cases at
576 x 576 x 88 and 85 cases at 640 x 640 x 88. The generator still emits
deterministic synthetic bytes (never dataset bytes), but it now reproduces the
real shape/dtype profiles so the stub can be exercised at both sizes.
...
PROFILE_SPECS = {
    "576x576x88": {"shape": (576, 576, 88), "dtype": "uint8", "case_count": 69},
    "640x640x88": {"shape": (640, 640, 88), "dtype": "uint8", "case_count": 85},
}
DEFAULT_PROFILE = "576x576x88"
SEED = 2024                          # the project's canonical seed


def synth_volume(shape, seed: int = SEED, dtype: str = "uint8") -> np.ndarray:
    """Deterministic volume with structure, not noise.
    ...
    scale = 900.0 if dtype == "int16" else 180.0
    texture = 120.0 if dtype == "int16" else 24.0
    noise = 25.0 if dtype == "int16" else 5.0
    base = np.clip(1.0 - r, 0.0, 1.0) * scale
    base += texture * np.sin(x / 9.0) * np.cos(y / 11.0)
    base += rng.normal(0.0, noise, size=(nx, ny, nz)).astype(np.float32)
    return np.clip(base, 0, high).astype(np.dtype(dtype))
...
def build(out_dir: str, shape, mesh_levels_json: str | None,
          dtype: str = "uint8", profile_name: str | None = None,
          case_count: int | None = None) -> dict:
    os.makedirs(out_dir, exist_ok=True)
    nx, ny, nz = shape

    vol = synth_volume(shape, dtype=dtype)
    mask = synth_mask(shape)

    # whole volume, raw bytes (strategy 3). Dtype is explicit in the manifest;
    # the generated bytes are synthetic and are never committed.
    vol_path = os.path.join(out_dir, f"volume_{dtype}.raw")
    ...
    manifest = {
        ...
        "_substitutions_recorded": [
            "Volume is SYNTHETIC. Spike D delivered shape/dtype profile metadata, but not "
            "dataset bytes; TASK.md permits synthetic bytes so Spike E does not wait.",
            "Shape and dtype mirror Spike D A6 profiles, but the bytes remain synthetic and "
            "must never be mistaken for the validated cohort.",
        ...
        "dtype": dtype,
        "case_count_observed": case_count,
...
def main() -> int:
    ...
    ap.add_argument("--shape", default=None,
                    help="Nx,Ny,Nz for one custom profile (legacy/single-profile mode)")
    ap.add_argument("--dtype", choices=["uint8", "int16"], default="uint8",
                    help="dtype for --shape custom mode; real Spike D profiles are uint8")
    ap.add_argument("--profiles", default=",".join(PROFILE_SPECS),
                    help="comma-separated real profiles to generate (default: both A6 profiles)")
    ...
    if args.shape:
        profiles = {"custom": {"shape": tuple(int(v) for v in args.shape.split(",")),
                                "dtype": args.dtype, "case_count": None}}
    else:
        names = [n.strip() for n in args.profiles.split(",") if n.strip()]
        unknown = [n for n in names if n not in PROFILE_SPECS]
        if unknown:
            ap.error(f"unknown profile(s): {', '.join(unknown)}")
        profiles = {n: PROFILE_SPECS[n] for n in names}

    root = os.path.abspath(args.out)
    os.makedirs(root, exist_ok=True)
    summaries = {}
    for name, spec in profiles.items():
        profile_out = root if name == "custom" else os.path.join(root, name)
        summaries[name] = build(profile_out, spec["shape"], args.mesh_levels,
                                dtype=spec["dtype"], profile_name=name,
                                case_count=spec.get("case_count"))

    if not args.shape:
        root_manifest = {
            "_status": "SYNTHETIC PAYLOAD PROFILES - not dataset bytes, not acceptance evidence",
            "source": "Spike D A6 observed profiles; generated bytes are deterministic synthetic stand-ins",
            "profile_manifest": "spikes/spike_e_transport/payloads/PROFILE_MANIFEST.json",
            "default_profile": DEFAULT_PROFILE,
            "profiles": {
                name: {"payload_dir": name, **summary}
                for name, summary in summaries.items()
            },
        }
        with open(os.path.join(root, "payload_manifest.json"), "w",
                  encoding="utf-8", newline="\n") as f:
            json.dump(root_manifest, f, indent=1, ensure_ascii=False)
            f.write("\n")
    ...
    print()
    print("  SYNTHETIC. Shape/dtype mirror Spike D A6; bytes are not dataset evidence.")
