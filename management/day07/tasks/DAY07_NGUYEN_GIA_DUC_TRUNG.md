...
|---|---|---|---|---|
| **1** | **Stub 2 profile — theo đường leader chọn sáng nay** (hẹn trước 10:00). **Phương án A — Project Control dựng bằng SSH của leader:** bạn ghi lên #26 **đúng lệnh cần chạy** (commit `a585907`, lệnh `payloads/generate.py` cho cả hai profile, lệnh `stub/server.py --payloads … --bind 10.64.193.115 --port 8787 --log <file>`) **trước 11:00** → PC chạy và dán output → **bạn kiểm**: `/health` liệt kê profile, `HEAD /s3/volume.raw?profile=576x576x88` và `?profile=640x640x88` trả **`Content-Length` khác nhau, đúng `PROFILE_MANIFEST.json`**, có `X-Payload-Profile`. Muốn kiểm trực tiếp trên địa chỉ điện thoại dùng (`10.64.193.115`) thì gửi **node ID ZeroTier máy bạn** cho leader duyệt vào mạng `b103a835d292ddb3`; không kịp thì PC dán output và bạn đối chiếu. **Phương án B — leader cấp khoá SSH cho bạn:** bạn tự dựng rồi kiểm như trên | ~1,5 h | **quyền:** SSH Mac mini (leader đang có) hoặc node ZeroTier được duyệt · leader chọn đường trước 10:00 | comment trên #26: lệnh + output từng profile — **trước 20:30** |
