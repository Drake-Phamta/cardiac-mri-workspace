...
|---|---|---|---|---|
| **1** | ✅ **STUB ĐÃ CHẠY 11:06 — PID 73790 trên `10.64.193.115:8787`** (phương án A: PC dựng bằng SSH của leader, worktree riêng tại `a585907`, payload 102 MB nằm **ngoài** repo, log `stub-20260916-net-b103a835.jsonl`). Kiểm ngay trên Mac mini: `/health` liệt kê **cả hai profile**, `HEAD` trả **29 196 288** và **36 044 800** với `X-Payload-Profile` đúng, byte tải về khớp `sha256_volume` trong manifest; **stub cũ PID 60294 không bị đụng**. Output đầy đủ ở comment trên #26. ⚠ Phát hiện kèm theo: **stub không hỗ trợ HTTP Range** (`curl -r` trả `200` + toàn bộ volume), và `client` trong log lúc kiểm là `10.64.193.115` — lúc leader đo phải là `10.64.193.140`. **Bạn giữ thiết kế, phép kiểm và mọi con số `E`**; chỉ phần *chạy lệnh trên máy* là của leader/PC, đúng `DR-006a` rev 2. **Phần của bạn:** (1) đọc output PC dán trên #26 và **đối chiếu với `PROFILE_MANIFEST.json`** — `576x576x88` phải ra `Content-Length` 29 196 288 và `640x640x88` ra 36 044 800, mỗi phản hồi có `X-Payload-Profile` đúng, `/health` liệt kê cả hai profile; (2) nếu muốn **tự kiểm trực tiếp** trên đúng địa chỉ điện thoại dùng (`10.64.193.115`), gửi **node ID ZeroTier máy bạn** cho leader duyệt vào mạng `b103a835d292ddb3`; (3) ghi xác nhận trên #26 — **trước 20:30** để leader kịp khung 21:00. Lệnh nào bạn muốn chạy khác đi thì ghi lên #26, PC chạy đúng lệnh đó | ~1 h | không còn chờ quyền — PC dựng trước 14:00 | comment xác nhận trên #26 **trước 20:30** |
| **2** | **Review PR #35** — split Path A thay #28, theo `DR-002a` **và `DR-002b` (leader quyết 16/09, phương án (c) + (d))**: `split.py --selftest` (11/11), JSON Schema, đếm **80/20/54**, holdout đúng 54 case `Testing Set`, **`CASE_0056` và `CASE_0097` cùng ở train**, tập con `20 ⊂ 40 ⊂ 80` chứa **cả hai hoặc không case nào**, chạy hai lần ra **cùng** file, từ chối nguồn chưa sẵn, `linkage_screen.py --selftest` (5/5). **Thêm ba phép kiểm của `DR-002b`:** (a) **ngưỡng tương quan được khai trong manifest trước mọi lượt train** — kiểm bằng lịch sử commit, không phải bằng lời; (b) luật gom nhóm và **loại case nghi trùng bệnh nhân với holdout** được áp **đồng đều**, danh sách loại có mã case + điểm; (c) số đếm trong manifest khớp danh sách đó. Không có bản ZIP thì ghi rõ phần nào **không** tái lập được thay vì bỏ qua | ~2,5 h | Khánh nhờ review | `APPROVE` hoặc `CHANGES_REQUESTED` có nội dung |
