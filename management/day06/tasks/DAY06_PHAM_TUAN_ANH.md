...
| **8** | ▶ **đang làm** *(Claude, worktree riêng `spike-a/s5-brush`; anh duyệt trước khi đẩy)* — **Spike A chặng S5 — brush** *(Claude + anh)*: ADD / ERASE, undo / redo, xuất mask để checksum; kiểm offline `A3` `A4` `A6` `A7` và chạy 60 ca `brush_cases.json` cho `A5` | ~3 h | không | PR nháp + kiểm offline đạt; đo trên máy sau |
