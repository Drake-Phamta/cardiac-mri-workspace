...
"""
import collections, hashlib, json, os, zipfile
import numpy as np
...
for c in man["cases"]:
    for role in ("mri", "mask"):
        groups[c[role]["sha256"]].append((c["case_id"], role))
...
for g in shared:
    ...
    for cid in ids:
        ...
        print(f"{cid} {rel.split('/')[0]} shape(z,y,x)={mri.shape} sha lgemri={s1[:12]} laendo={s2[:12]} lawall={s3[:12]}"
              f" | lgemri/laendo sha == manifest: {s1 == cases[cid]['mri']['sha256']}/{s2 == cases[cid]['mask']['sha256']}")
    a, b = ids[0], ids[1]
