...
| **6** | **Spike A chặng `S6`** — ✅ **dụng cụ đã dựng xong 06:47, APK release đã cài, chỉ còn anh bấm.** PC đã làm: ① cửa sổ cache ±3 **có unmount** + công tắc `all` \| `±3` ngay trong app · ② `generate.py --nz` → fixture **576×576×88 thật** *(78 MB JSON, conformance `F1`–`F4` qua)* · ③ `capture_conditions.py` đọc thẳng meminfo/pin/nhiệt/độ sáng/refresh · ④ `build_type` suy từ `__DEV__`, `cache_policy`/`nz`/chuỗi bước đọc từ log của app — **không còn trường nào gõ tay** · ⑤ build release xong, APK **140 MB**, cài lúc 06:40. **Việc của anh:** mở khoá máy, mở app, rồi chạy `python harness/measure_a9.py --policy all` và `--policy window` — script tự xoá buffer, bắt điều kiện trước/sau, chờ app báo `RUN_END`, rồi trích. Nó **từ chối chạy** nếu máy khoá hoặc app không ở foreground | ~40 ph *(còn lại)* | **anh mở khoá máy** | 2 bản ghi `a9_*` có `cache_policy` + `memory_delta` |
| **6b** | **Một số đã đo được mà chưa cần anh:** app mang fixture 88 slice tốn **204 MB native heap · 324 MB TOTAL PSS *trước khi decode một slice nào*** — đó là chi phí của *chính cách nạp dữ liệu*, tách khỏi chi phí cache. Trần heap máy là **256 MB** (`heapgrowthlimit`, khớp phát hiện Day 1). Lúc màn hình sáng được ~4 giây, graphics nhảy **0,26 → 73 MB** rồi tụt lại khi app bị đẩy xuống nền vì màn khoá. ⚠ **Chưa phải phép đo S6** — prewarm chưa chạy xong; ghi ở đây để khỏi bị nhớ nhầm thành kết quả | — | — | ghi vào `RESULT.md` khi có lượt đo đủ |
